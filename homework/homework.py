...
def agregar(dataFrame: pd.DataFrame) -> pd.DataFrame:
    df = dataFrame.copy()
    df["Age"] = 2025 - df["Year"]
...
def separar(df: pd.DataFrame):
    features = df.drop(columns=["Present_Price"])
    target = df["Present_Price"]
    return features, target


def crearPipeline(featureFrame: pd.DataFrame) -> Pipeline:
...
def tuneFit(pipe: Pipeline, xTrain, yTrain):
    paramGrid = {
...
def builds(yTrue, yPred, datasetLabel: str) -> dict:
    return {
        "type": "metrics",
...
def write(rows, filePath: str = "files/output/metrics.json"):
...
def runProgram():
    trainData, testData = cargardatos()

    trainData = agregar(trainData)
    testData = agregar(testData)

    xTrain, yTrain = separar(trainData)
    xTest, yTest = separar(testData)

    pipeline = crearPipeline(xTrain)
    fittedModel = tuneFit(pipeline, xTrain, yTrain)
    ...
    trainMetrics = builds(yTrain, trainPred, "train")
    testMetrics = builds(yTest, testPred, "test")

    write([trainMetrics, testMetrics])
